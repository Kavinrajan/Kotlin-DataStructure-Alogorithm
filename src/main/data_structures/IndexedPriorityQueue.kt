...
* Data type of key, Data type of value, Comparator, Hash function
* */
...
class IndexedPriorityQueue<T>(size: Int, val comparator: Comparator<T>? = null): Collection<T> {
    ...
    fun decreaseKey(i:Int, key: T) {
        ...
        if(!greater(keys[i]!!, key))
            throw IllegalArgumentException("Calling deceraseKey()" +
    ...
    private fun lesser(x: T, y: T): Boolean {
